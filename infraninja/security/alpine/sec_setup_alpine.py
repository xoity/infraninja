...
def setup_alpine():
    ...
    if tools_to_install:
        apk.packages(name="Install security tools", packages=tools_to_install)
    ...
    server.shell(
        name="Ensure media encryption", commands=[f"cryptsetup luksFormat {media}"]
    )
    ...
    if security_tools["suricata"]:
        suricata_config_path = (
            "/home/xoity/Desktop/infraninja/test/configs/alpine/suricata/suricata.yaml"
        )
        if not exists(suricata_config_path):
            print(
                f"Suricata configuration file not found at {suricata_config_path}. Please ensure it exists."
            )
        else:
            files.put(
        ...
        server.shell(
            ...
            commands=[
                "suricata -c /etc/suricata/suricata.yaml -i eth0 -D > /var/log/suricata/suricata.log"
            ],
        )
...
def set_acls():
    ...
    for path, acl_rule in ACL_PATHS.items():
        server.shell(
            name=f"Set ACL for {path}", commands=[f"setfacl -m {acl_rule} {path}"]
        )
