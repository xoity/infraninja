...
def format_motd() -> str:
    ...
    if history.data.get('last_access'):
        ...
        content.append(f"\nLast Access: {last_access.strftime('%Y-%m-%d %H:%M:%S')}")
        
        if history.data.get('selected_groups'):
            content.append(f"Selected Groups: {', '.join(history.data['selected_groups'])}")
...
def show_motd(hostname: Optional[str] = None, 
